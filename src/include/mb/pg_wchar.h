 ...
 *
 *	NOTES
 *		This is used both by the backend and by frontends, but should
 * not be included by libpq client programs.  In particular, a libpq client
 *		should not assume that the encoding IDs used by the version of
 * libpq it's linked to match up with the IDs declared here. To help prevent
 * mistakes, relevant functions that are exported by libpq have a physically
 * different name when being referenced statically.
 *
 *-------------------------------------------------------------------------
 */
#ifndef PG_WCHAR_H
...
typedef enum EncodingValidationPolicy {
  ...
  ENCODING_VALIDATION_READ_COMPATIBLE_LOOKUP,
 ...
 * src/common/encnames.c) and the pg_wchar_table[] array (in src/common/wchar.c)
 * and to check PG_ENCODING_BE_LAST macro.
 *
...
static inline pg_wchar surrogate_pair_to_codepoint(pg_wchar first,
                                                   pg_wchar second) {
 ...
 */
static inline unsigned char *unicode_to_utf8(pg_wchar c,
 ...
 */
static inline int unicode_utf8len(pg_wchar c) {
  if (c <= 0x7F)
    return 1;
  else if (c <= 0x7FF)
    return 2;
...
extern unsigned char *pg_do_encoding_conversion(unsigned char *src, int len,
                                                int src_encoding,
                                                int dest_encoding);
extern int pg_do_encoding_conversion_buf(Oid proc, int src_encoding,
                                         int dest_encoding, unsigned char *src,
