...
static pg_attribute_always_inline bool
NextCopyFromRawFieldsInternal(CopyFromState cstate, char ***fields,
                              int *nfields, bool is_csv);

/* Low-level communications functions */
static int CopyGetData(CopyFromState cstate, void *databuf, int minread,
                       int maxread);
      ...
      readmessage:
        ...
        if (mtype == EOF)
          ereport(ERROR, (errcode(ERRCODE_CONNECTION_FAILURE),
                          errmsg("unexpected EOF on client connection with an "
                                 "open transaction")));
...
static void CopyConvertBuf(CopyFromState cstate) {
  ...
  if (!cstate->need_transcoding) {
    ...
    ereport(NOTICE, (errmsg("copy encoding valiation = %d",
                            copy_encoding_validation_policy)));
    nverified = pg_encoding_verifymbstr(
        cstate->file_encoding, cstate->raw_buf + preverifiedlen, unverifiedlen);
    if (nverified == 0) {
      /*
    ...
    convertedlen = pg_do_encoding_conversion_buf(
        cstate->conversion_proc, cstate->file_encoding, GetDatabaseEncoding(),
...
static void CopyConversionError(CopyFromState cstate) {
  ...
  if (!cstate->need_transcoding) {
    ...
    (void)pg_do_encoding_conversion_buf(
        cstate->conversion_proc, cstate->file_encoding, GetDatabaseEncoding(),
...
static void CopyLoadRawBuf(CopyFromState cstate) {
  ...
  /* Load more data */
  inbytes = CopyGetData(cstate, cstate->raw_buf + cstate->raw_buf_len, 1,
  ...
  if (cstate->cur_lineno == 0 && cstate->opts.header_line) {
    ...
    if (cstate->opts.header_line == COPY_HEADER_MATCH) {
      ...
      foreach (cur, cstate->attnumlist) {
        ...
        if (colName == NULL)
          ereport(ERROR, (errcode(ERRCODE_BAD_COPY_FILE_FORMAT),
                          errmsg("column name mismatch in header line field "
                                 "%d: got null value (\"%s\"), expected \"%s\"",
                                 fldnum, cstate->opts.null_print,
 ...
 */
bool NextCopyFrom(CopyFromState cstate, ExprContext *econtext, Datum *values,
                  bool *nulls) {
  TupleDesc tupDesc;
  AttrNumber num_phys_attrs, num_defaults = cstate->num_defaults;
  int i;
  ...
  for (i = 0; i < num_defaults; i++) {
    ...
    values[defmap[i]] =
        ExecEvalExpr(defexprs[defmap[i]], econtext, &nulls[defmap[i]]);
  }
...
bool CopyFromTextOneRow(CopyFromState cstate, ExprContext *econtext,
...
bool CopyFromCSVOneRow(CopyFromState cstate, ExprContext *econtext,
                       Datum *values, bool *nulls) {
...
CopyFromTextLikeOneRow(CopyFromState cstate, ExprContext *econtext,
  ...
  foreach (cur, cstate->attnumlist) {
     ...
     */
                                    ...
                                    att->atttypmod, (Node *)cstate->escontext,
                                    &values[m])) {
      ...
      if (cstate->opts.log_verbosity == COPY_LOG_VERBOSITY_VERBOSE) {
        ...
        if (cstate->cur_attval) {
          ...
          ereport(NOTICE,
                  errmsg("skipping row due to data type incompatibility at "
                         "line %" PRIu64 " for column \"%s\": \"%s\"",
                         cstate->cur_lineno, cstate->cur_attname, attval));
          pfree(attval);
        } else
          ereport(NOTICE,
                  errmsg("skipping row due to data type incompatibility at "
                         "line %" PRIu64 " for column \"%s\": null input",
...
bool CopyFromBinaryOneRow(CopyFromState cstate, ExprContext *econtext,
                          Datum *values, bool *nulls) {
  TupleDesc tupDesc;
  AttrNumber attr_count;
  FmgrInfo *in_functions = cstate->in_functions;
  ...
  if (fld_count != attr_count)
    ereport(ERROR, (errcode(ERRCODE_BAD_COPY_FILE_FORMAT),
                    errmsg("row field count is %d, expected %d", (int)fld_count,
                           attr_count)));
  ...
  foreach (cur, cstate->attnumlist) {
    ...
    cstate->cur_attname = NameStr(att->attname);
    values[m] = CopyReadBinaryAttribute(
        cstate, &in_functions[m], typioparams[m], att->atttypmod, &nulls[m]);
    cstate->cur_attname = NULL;
  }
  ...
  for (;;) {
      ...
      if (cstate->eol_type == EOL_UNKNOWN || cstate->eol_type == EOL_CRNL) {
        ...
        if (c == '\n') {
          ...
          if (cstate->eol_type == EOL_CRNL)
            ereport(ERROR,
                    (errcode(ERRCODE_BAD_COPY_FILE_FORMAT),
                     !is_csv ? errmsg("literal carriage return found in data")
                             : errmsg("unquoted carriage return found in data"),
                     !is_csv
                         ? errhint("Use \"\\r\" to represent carriage return.")
                         : errhint("Use quoted CSV field to represent carriage "
                                   "return.")));
    ...
    if (c == '\n' && (!is_csv || !in_quote)) {
      if (cstate->eol_type == EOL_CR || cstate->eol_type == EOL_CRNL)
        ereport(ERROR,
                (errcode(ERRCODE_BAD_COPY_FILE_FORMAT),
                 !is_csv ? errmsg("literal newline found in data")
    ...
    if (c == '\\' && !is_csv) {
      ...
      if (c2 == '.') {
        ...
        if (cstate->eol_type == EOL_CRNL) {
          ...
          if (c2 == '\n')
            ereport(ERROR, (errcode(ERRCODE_BAD_COPY_FILE_FORMAT),
                            errmsg("end-of-copy marker does not match previous "
                                   "newline style")));
          else if (c2 != '\r')
            ereport(ERROR,
                    (errcode(ERRCODE_BAD_COPY_FILE_FORMAT),
         ...
         */
        if (cstate->line_buf.len > 0 || prev_raw_ptr > cstate->input_buf_index)
          ereport(ERROR,
                  (errcode(ERRCODE_BAD_COPY_FILE_FORMAT),
                   errmsg("end-of-copy marker is not alone on its line")));
...
static Datum CopyReadBinaryAttribute(CopyFromState cstate, FmgrInfo *flinfo,
  ...
  if (CopyReadBinaryData(cstate, cstate->attribute_buf.data, fld_size) !=
      fld_size)
    ereport(ERROR, (errcode(ERRCODE_BAD_COPY_FILE_FORMAT),
                    errmsg("unexpected EOF in COPY data")));
