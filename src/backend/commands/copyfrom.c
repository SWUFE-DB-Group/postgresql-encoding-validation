 ...
 * copyfromparse.c.
 */
static void CopyFromTextLikeInFunc(CopyFromState cstate, Oid atttypid,
...
static void CopyFromTextLikeEnd(CopyFromState cstate) { /* nothing to do */ }
...
static void CopyFromBinaryEnd(CopyFromState cstate) { /* nothing to do */ }
...
void CopyFromErrorCallback(void *arg) {
  ...
  if (cstate->opts.binary) {
    ...
    if (cstate->cur_attname)
      errcontext("COPY %s, line %" PRIu64 ", column %s", cstate->cur_relname,
                 cstate->cur_lineno, cstate->cur_attname);
    else
      errcontext("COPY %s, line %" PRIu64, cstate->cur_relname,
                 cstate->cur_lineno);
  } else {
    if (cstate->cur_attname && cstate->cur_attval) {
      ...
      errcontext("COPY %s, line %" PRIu64 ", column %s: \"%s\"",
                 cstate->cur_relname, cstate->cur_lineno, cstate->cur_attname,
                 attval);
      pfree(attval);
      ...
      if (cstate->line_buf_valid) {
        ...
        errcontext("COPY %s, line %" PRIu64 ": \"%s\"", cstate->cur_relname,
...
static inline void CopyMultiInsertBufferFlush(CopyMultiInsertInfo *miinfo,
  ...
  if (resultRelInfo->ri_FdwRoutine) {
    ...
    while (sent < nused) {
      ...
      rslots = resultRelInfo->ri_FdwRoutine->ExecForeignBatchInsert(
          estate, resultRelInfo, &slots[sent], NULL, &inserted);
    ...
    table_multi_insert(resultRelInfo->ri_RelationDesc, slots, nused, mycid,
                       ti_options, buffer->bistate);
    ...
    for (i = 0; i < nused; i++) {
      ...
      if (resultRelInfo->ri_NumIndices > 0) {
        ...
        ExecARInsertTriggers(estate, resultRelInfo, slots[i], recheckIndexes,
                             cstate->transition_capture);
       ...
       */
        ...
        cstate->cur_lineno = buffer->linenos[i];
        ExecARInsertTriggers(estate, resultRelInfo, slots[i], NIL,
                             cstate->transition_capture);
      }
 ...
 */
static inline void CopyMultiInsertInfoStore(CopyMultiInsertInfo *miinfo,
                                            ResultRelInfo *rri,
                                            TupleTableSlot *slot, int tuplen,
                                            uint64 lineno) {
  CopyMultiInsertBuffer *buffer = rri->ri_CopyMultiInsertBuffer;
...
uint64 CopyFrom(CopyFromState cstate) {
  ...
  if (cstate->rel->rd_rel->relkind != RELKIND_RELATION &&
    ...
    if (cstate->rel->rd_rel->relkind == RELKIND_VIEW)
      ereport(ERROR, (errcode(ERRCODE_WRONG_OBJECT_TYPE),
                      ...
                      errhint("To enable copying to a view, provide an INSTEAD "
                              "OF INSERT trigger.")));
  ...
  if (cstate->opts.freeze) {
    ...
    if (!ThereAreNoPriorRegisteredSnapshots() || !ThereAreNoReadyPortals())
      ereport(ERROR, (errcode(ERRCODE_INVALID_TRANSACTION_STATE),
                      errmsg("cannot perform COPY FREEZE because of prior "
    ...
    if (cstate->rel->rd_createSubid != GetCurrentSubTransactionId() &&
      ...
      ereport(ERROR,
              (errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
               errmsg("cannot perform COPY FREEZE because the table was not "
  ...
  cstate->transition_capture = mtstate->mt_transition_capture =
      MakeTransitionCaptureState(cstate->rel->trigdesc,
                                 RelationGetRelid(cstate->rel), CMD_INSERT);
  ...
  if (cstate->whereClause)
    cstate->qualexpr =
        ExecInitQual(castNode(List, cstate->whereClause), &mtstate->ps);
  ...
  if (resultRelInfo->ri_TrigDesc != NULL &&
    ...
    CopyMultiInsertInfoInit(&multiInsertInfo, resultRelInfo, cstate, estate,
                            mycid, ti_options);
  }
  ...
  for (;;) {
    ...
    if (cstate->opts.on_error == COPY_ON_ERROR_IGNORE &&
      ...
      if (cstate->opts.reject_limit > 0 &&
        ...
        ereport(ERROR, (errcode(ERRCODE_INVALID_TEXT_REPRESENTATION),
                        errmsg("skipped more than REJECT_LIMIT (%" PRId64
    ...
    if (proute) {
       ...
       */
      resultRelInfo = ExecFindPartition(mtstate, target_resultRelInfo, proute,
                                        myslot, estate);
      ...
      if (insertMethod == CIM_SINGLE || !leafpart_use_multi_insert) {
        ...
        batchslot =
            CopyMultiInsertInfoNextFreeSlot(&multiInsertInfo, resultRelInfo);
    ...
    if (!skip_tuple) {
      ...
      if (has_instead_insert_row_trig) {
        ...
        if (insertMethod == CIM_MULTI || leafpart_use_multi_insert) {
          ...
          /* OK, store the tuple */
          if (resultRelInfo->ri_FdwRoutine != NULL) {
            myslot = resultRelInfo->ri_FdwRoutine->ExecForeignInsert(
                estate, resultRelInfo, myslot, NULL);
            ...
            table_tuple_insert(resultRelInfo->ri_RelationDesc, myslot, mycid,
                               ti_options, bistate);

            if (resultRelInfo->ri_NumIndices > 0)
              recheckIndexes =
                  ExecInsertIndexTuples(resultRelInfo, myslot, estate, false,
                                        false, NULL, NIL, false);
          }

          /* AFTER ROW INSERT Triggers */
          ExecARInsertTriggers(estate, resultRelInfo, myslot, recheckIndexes,
                               cstate->transition_capture);
  ...
  if (cstate->opts.on_error != COPY_ON_ERROR_STOP && cstate->num_errors > 0 &&
      cstate->opts.log_verbosity >= COPY_LOG_VERBOSITY_DEFAULT)
    ereport(NOTICE,
            errmsg_plural("%" PRIu64
  ...
  ExecASInsertTriggers(estate, target_resultRelInfo,
                       cstate->transition_capture);
 ...
 */
CopyFromState BeginCopyFrom(ParseState *pstate, Relation rel, Node *whereClause,
  ...
  int64 progress_vals[] = {PROGRESS_COPY_COMMAND_FROM, 0, 0};
    ...
    foreach (cur, attnums) {
      ...
      if (!list_member_int(cstate->attnumlist, attnum))
        ereport(ERROR, (errcode(ERRCODE_INVALID_COLUMN_REFERENCE),
                        /*- translator: first %s is the name of a COPY option,
                           e.g. FORCE_NOT_NULL */
  ...
  if (cstate->file_encoding == GetDatabaseEncoding() ||
    ...
    if (!OidIsValid(cstate->conversion_proc))
      ereport(ERROR, (errcode(ERRCODE_UNDEFINED_FUNCTION),
                      errmsg("default conversion function for encoding \"%s\" "
                             "to \"%s\" does not exist",
  ...
  pgstat_progress_start_command(PROGRESS_COMMAND_COPY,
                                cstate->rel ? RelationGetRelid(cstate->rel)
                                            : InvalidOid);
...
static void ClosePipeFromProgram(CopyFromState cstate) {
  ...
  if (pclose_rc == -1)
     ...
     * problem.
     */
    if (!cstate->raw_reached_eof && wait_result_is_signal(pclose_rc, SIGPIPE))
      return;
